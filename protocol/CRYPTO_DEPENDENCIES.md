...
The first maintenance task only resolved recorded sources and missed the subsequent JSON serialization source request. The corrected `gradle/verify-ide-sources.init.gradle` also discovers sources from JVM compile/runtime/test classpaths using Gradle's artifact-resolution query. It reproduced seven missing source checksums before the fix: JSON JVM/common, serialization core/common and CBOR/common, coroutines/common, JUnit and Hamcrest. All seven cached files matched fresh Maven Central downloads before their hashes were added. It now verifies source resolution for 17 JVM dependency components and all 123 recorded source JARs (including exact build-tool variant classifiers). `verifyIdeSources` and `:app:assembleDebug` passed with strict verification after reproducing that failure. This is not a run of Android Studio's full sync model. Additional IDE artifact types remain fail-closed until reviewed; no trusted-artifact wildcard or source exclusion is used.
