...
For a 30-second message, the sender counts from locally committed server acceptance and the receiver from decrypt/commit, not reading. Leave one endpoint offline before send to confirm Pending has no countdown, then reconnect. A pending policy row explains that it applies locally while retrying; permanent outbox failure requires selecting the timer again. Initial controls do not bypass an unaccepted request: send a normal message and explicitly accept before the receiver applies a deferred control.
