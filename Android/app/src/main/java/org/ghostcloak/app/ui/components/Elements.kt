...
import org.ghostcloak.app.ui.theme.Warning
...
@Composable fun Avatar(name: String, modifier: Modifier = Modifier) {
    Surface(modifier.size(52.dp), shape = CircleShape, color = MaterialTheme.colorScheme.surfaceVariant) {
...
@Composable fun TrustBadge(state: IdentityTrustState?) {
    ...
    val color = when (state) { IdentityTrustState.VERIFIED -> MaterialTheme.colorScheme.primary; IdentityTrustState.CHANGED -> MaterialTheme.colorScheme.error; else -> Warning }
...
@Composable fun InfoPanel(title: String, body: String, warning: Boolean = false, urgent: Boolean = false) {
    Surface(shape = MaterialTheme.shapes.medium, border = if (urgent) BorderStroke(2.dp, MaterialTheme.colorScheme.error) else null,
        ...
        Column(Modifier.fillMaxWidth().padding(20.dp), verticalArrangement = Arrangement.spacedBy(7.dp)) {
            Text(title, style = if (urgent) MaterialTheme.typography.headlineSmall else MaterialTheme.typography.titleMedium,
...
@Composable fun ScreenHeader(title: String, subtitle: String, back: (() -> Unit)? = null) {
    if (back != null) TextButton(onClick = back, contentPadding = PaddingValues(0.dp)) { Text("‹  Back") }
