...
## Implemented Phase 1H.2 expiration interaction

The existing uniquely named 30-minute/15-minute-flex mailbox work and connectivity constraint are unchanged. AppRuntime reconciles local expiry around its serialized operations, including worker execution and startup. A single process-owned local 1-second cleanup loop additionally runs without network calls while execution is available (including app lock, offline and explicit logout); it does not keep the process alive and introduces no alarm, wake lock, service or additional WorkManager job. Before first device unlock, credential-protected storage remains unavailable and is not accessed. Force-stop/Doze are respected.

Expired ordinary records and read/notification eligibility are removed transactionally; deduplication and Signal state remain. Policy controls create no notification eligibility. If Android delays execution, repository reads and the private UI deadline filter prevent expired content from appearing on resume/unlock, and the next permitted cleanup cancels an otherwise empty aggregate. The generic notification stays exactly Ghost Cloak / New message with no timer, sender or preview. Notification removal while Android suspends the process is best-effort. No server-visible timer or scheduling input is introduced.
