...
“Connected” reflects this app's last successful operation, never another person's presence. While the app is visible, automatic sync runs immediately on resume and then four seconds after each completed cycle. It reuses authentication, renews on expiry, retries pending sends and fetches messages/receipts. **Sync** remains available for recovery. Backgrounding stops the loop; logout disables automatic reconnection until explicitly connected again.
