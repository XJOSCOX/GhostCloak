...
## Confirmed trace and implemented correction

Phone A subsequently confirmed identity present, registration/token absent, device credential present and renewal blocked; Connect reached AUTH_VERIFY/401 then ACCOUNT_REGISTER/409. The ambiguous-state fallback below was therefore exercised. The antecedent metadata loss/contamination remains unknown, and key-entry presence alone does not prove a server match.

The correction removes registration fallback for existing/ambiguous state. Valid existing login still repairs a missing marker; missing token with registered metadata still uses ordinary login. Failed existing login becomes RECOVERY_REQUIRED. Only a durable intent created when this build explicitly creates a brand-new local identity authorizes initial registration. The prepared registration is persisted for retries. A pending first-registration intent may retry its original registration after login cannot confirm an earlier lost response: the durable new-identity intent, not a 401, authorizes that path. Older ambiguous unfinished onboarding without this intent fails closed rather than being guessed new.

Recovery requires explicit unlocked foreground action. The existing non-exportable device-auth key signs a one-time, at-most-60-second challenge with the existing DeviceAuth transcript encoding. It includes version, nonce, challenge ID, expiry, server audience, purpose `recover`, original local device ID and SHA-256 of the auth public key in the existing registration-hash field. The account field is an unrelated random transcript value, not an existing binding or contaminated local candidate. Issuance never looks up the credential. Verification scans the bounded device set, verifies the original stored auth public key, enforces the exact original device ID, and only then returns original account/device/routing IDs and a fresh single-active-session token. No username, Signal material or ownership claim is accepted as proof; no server key or ownership is changed.

Known/unknown valid public keys have identical challenge shapes. Unknown key, invalid signature, wrong binding and expired/replayed proof receive uniform 401 `recovery_failed`. Each recovery endpoint has a separate global anonymous ten-request/minute limit. This is conservative staging protection, not resistance to denial of service. No absolute constant-time network guarantee is claimed: malformed inputs can fail structural validation and database/network scheduling varies. No result reveals credential existence before valid proof.

After validating the complete proof response, one SQLCipher transaction restores only account/routing metadata, registered marker and fresh token; removes stale pending/new-account intent, recovery-required/logout flags and renewal block. It rechecks the same stored public key/Keystore entry and original local device ID. Signal material, contacts, messages, verification, policies, outbox, attachments and app lock are untouched. A visibility/lock/logout change prevents commit. A crash before commit requires a fresh challenge; after commit, restart opens the original registered binding. Consumed proofs never replay. WorkManager and polling never initiate recovery.

Account states are NEW_ACCOUNT, REGISTERED, RECOVERY_REQUIRED, RECOVERING, RECOVERED and LOGGED_OUT. Durable explicit logout is distinct from a rejected-renewal block and suppresses background reconnect. Historical renewal-block flags remain conservative rather than silently authorizing recovery. Missing/unusable private key, unknown/mismatched server key, unsupported legacy credentials or changed local device ID fail closed with “Account recovery could not be verified.”

Backend release and V004 (challenge-purpose constraint only) are required; see [manual deployment](../infrastructure/ACCOUNT_RECOVERY_DEPLOYMENT.md). Nothing was deployed remotely. Attachment configuration and behavior are unchanged. **The following sections are the historical diagnostic record, not the corrected algorithm.**

Current correction validation: 135 JVM tests, 12 isolated PostgreSQL tests, full app/storage emulator regressions and the final three focused recovery emulator tests passed with strict dependency verification; debug/release builds and backend distribution passed. Reproduction now restores contaminated bindings using original-key proof while retaining real conversation history and communicating with a healthy second client. Failed local writes roll back the binding transaction. Tests and migration are committed with this correction. No physical-phone recovery or server deployment result is claimed.
